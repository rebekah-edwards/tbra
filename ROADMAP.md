...
- **Cover "coming soon" placeholder sweep** — similar to the ISBNdb + Google Books placeholder hashes already handled by `nightly-placeholder-clear`. Need to fingerprint the "cover coming soon" variant, add it to `PLACEHOLDERS` array in `src/lib/cover-placeholders.ts`, run a cleanup pass, and land on `/admin/covers` queue. Surfaced 2026-04-20 via `/book/the-ending-writes-itself` which had the placeholder and a slug missing author (slug fix already handled by triage rewrite).
...
- **Proposed edits flow — tightening** (2026-04-20): the current review wizard's content-details step asks the user for every category. Add a "these look correct as-is" confirmation path so a reviewer isn't forced to re-enter every rating they already agree with. Tighten the submit → `/admin/corrections` queue → apply flow so changes actually land. (Complaint surfaced while discussing book page perf.)
- **Search improvements** (2026-04-20): "Invisible Woman" case — Meilisearch relevance surfaces the wrong entries first. Needs a pass similar to the `words/exactness/typo/proximity/attribute/sort` rule tuning we did for "The Giver" in Round 11. Use Invisible Woman as the canonical test case.
- **Bottom-of-page padding on mobile — GLOBAL** (2026-04-20): profile page fix shipped, but every other page still runs content too close to the bottom nav. Audit `src/app/**/page.tsx` + the root layout — one shared `pb-24 lg:pb-8` wrapper rather than per-page copies.
- **sync-push hardening** (2026-04-20): the sync-push.ts process hung silently for 2h 9m on 2026-04-20, holding Turso connections and degrading book page loads to 40s+. Add a per-query timeout (~30s), a stall-detector (no progress in N minutes → abort), and surface progress via stdout so a stuck state is obvious. Logging here so we don't forget.
