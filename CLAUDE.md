...
- `nightly-description-refresh` — 4:04 AM PT, **Sun/Tue/Thu only** (`0 4 * * 0,2,4`): fills missing/junk descriptions, up to `BATCH_SIZE` (default 2000) per night. Script hardened 2026-06-20: watchdog-exempt + self-governed `MAX_RUNTIME_MIN` ceiling (SKILL sets 140 since it runs solo on its nights), exits 0 so the chained push always runs, and an API circuit breaker that stops on `API_EXHAUSTED`/auto-pause instead of spinning. **Dropped Sat on 2026-06-20** to free Sat for `breadth-import`. **Reworked 2026-08-08** after an audit found the lane structurally unable to fix the problem: (a) it selected `description_stale=1` ONLY, but 11,659 public books had no description and just 4,377 carried the flag — the other 7,281 were invisible to every job in the system, so the selector now takes `description_stale=1 OR description IS NULL OR TRIM(description)=''`, ordered user-shelved first; (b) **Brave + Grok are now OFF by default** (`skipContentSearch:true`) — measured on one fixed set of 10 aged books, the Brave+Grok path fixed 2/10 at 35.8s each while the free tiers + the new Google Books description tier fixed 3/12 at 3.9s each with zero Brave, i.e. the expensive path was both slower and lower-yield; set `USE_BRAVE=1` to run it deliberately against the residue; (c) `CONCURRENCY` (default 5, capped by ISBNdb's 3 req/sec) replaced the serial loop, which finished only ~175 of a 500-book batch per night. Net effect measured on a 60-book sample: 65s and 43 fixed, vs ~36 min and ~16 fixed under the old path. This freed ~2,500 Brave/night on Sun/Tue/Thu that `nightly-content-ratings-backfill` could claim. Companion: `scripts/sweep-junk-descriptions.ts` (dry-run by default) flags present-but-wrong descriptions so this lane replaces them.
- `nightly-content-ratings-backfill` — 5:38 AM ET, **Mon/Wed/Fri/Sat** (`30 5 * * 1,3,5,6`): **Slot moved from 4:45 on 2026-08-14.** `push-content-ratings-to-turso.ts` gates on 3:00–6:00 AM **Pacific** while the host clock and every cron slot here are **Eastern** — off the old slot a normal ~1h enrichment hit the push at ~2:50 AM PT and exited 3 on every run, so the night's ratings never left the local DB. The new slot puts a typical night's push at ~3:40 AM PT (150-min worst case ≈5:14 AM PT, still inside), and `nightly-content-ratings.sh` now also waits for the window if enrichment finishes early. **Any new PT-gated script needs this same ET→PT conversion done explicitly** — the two zones are 3h apart and every slot in this file is local ET. **450** Grok enrichments/night (`BACKFILL_LIMIT=450` set in the task command; trimmed from 500 on 2026-07-17 to preserve shared-Brave headroom for daytime user adds — see Brave section). **Expanded from Mon/Fri → Mon/Wed/Fri/Sat on 2026-07-07**: once discovery+breadth went Brave-free (`skipContentSearch`), ~3,000 Brave/night was freed, so this — now the PRIMARY source of content ratings for the whole catalog (bulk ingestion defers ratings here) — runs 4 nights instead of 2 (~2,000 books/week). Wed/Sat are safe because `breadth-import` no longer spends Brave. Still disjoint from `nightly-description-refresh` (Sun/Tue/Thu), which also fires ~5 Brave/book. ~2,700 Brave/night (450 × ~6) + thin-recovery's ~300-360 leaves ~1,000/day headroom under the shared 4,200 cap. NOTE: filters `visibility='public'`, so import_only (search-added) books are handled by `scripts/fix-shelf-enrichment.ts`, not here. **Reworked 2026-08-08** after an audit found the lane finishing only ~150 of 450 books/night: (a) it now runs `CONCURRENCY` (default 4) books in flight — the serial loop spent ~60s of wall clock per book against the 150min ceiling and lost two thirds of every batch, while spending only ~900 of the 8,000 Brave/day cap, so throughput was the limit, not budget; (b) the candidate pool now includes **thin** books (>=50% "no evidence found"), sent with `force:true` (the trigger skips already-rated books otherwise) and carrying the same 21-day `enrichment_log` success cooldown `recover-thin-ratings.ts` uses — ~20k public books were invisible to the old `NOT IN book_category_ratings` query; (c) ordering is user-shelved → NYT bestseller (ISBN-13, titleKey fallback) → newest-first, replacing `ORDER BY users DESC, b.title`, which went alphabetical for ~96% of each batch. `DRY_RUN=1` prints the plan without spending budget. `push-content-ratings-to-turso.ts` also mirrors the run's `enrichment_log` successes so thin-recovery (6:44 AM) doesn't re-enrich what this lane just did.
...
- **Retired tasks were DELETED from the scheduler on 2026-08-08**, per the "delete, don't disable" rule below: `process-reported-issues` (legacy, superseded by `nightly-report-triage`), `nightly-cover-rescue` (renamed to `nightly-placeholder-clear` 2026-04-17), `nightly-nyt-backfill` (retired 2026-06-20, replaced by `breadth-import`), and three one-time tasks (`slug-collision-resume` 04-21, `cron-proof-2026-05-04`, `verify-discovery-rebuild-2026-06-02`). All six had been sitting disabled and cluttering the sidebar. Their `SKILL.md` files remain on disk under `~/.claude/scheduled-tasks/<id>/` if a prompt needs recovering.
...
### com.tbra.housekeeping — hourly Mac-health chores (2026-08-24)
`com.tbra.housekeeping` (hourly at :47) → `cron-run.sh housekeeping scripts/lib/housekeeping.sh`,
logging to `data/housekeeping.log`. Two mechanical chores, no model:

1. **Reap leftover booted simulators.** A headless sim left `Booted` from an earlier build session
   (165 procs, 5.6GB) drove swap to 14.2/15.4GB and wedged the dev server mid-compile, breaking
   Rebekah's Tailscale-only tester app while TestFlight users were fine (memory
   `project_devserver_wedged_by_idle_simulator`). Reaping is guarded three ways — skipped entirely
   if the Simulator GUI is open, if `xcodebuild` is running, or if `simctl install/launch` is in
   flight — and only touches devices idle ≥60 min (`SIM_IDLE_MIN`), so it can never kill a sim a
   build or a human is using.
2. **Rotate `data/devserver.log`** at 20MB, keeping the last 2MB in `devserver.log.1`. It had grown
   unbounded to 37MB. **TRAP: this log MUST be truncated in place (`: > file`), never renamed.**
   launchd and the next-server processes hold persistent `O_APPEND` fds on the inode (verified
   2026-08-24), so `mv` leaves them writing to the orphaned inode and `devserver.log` never
   reappears until the service restarts — which is exactly what `cron-run.sh`'s own 5MB rotation
   does to its logs, where it is safe only because those are reopened per write.
...
- **nightly-placeholder-clear** (named `nightly-cover-rescue` before 2026-04-17) clears ISBNdb placeholder covers and sets `cover_source='isbndb-placeholder-cleared'`. Book then waits for manual review.
...
- **Google Books:** 1,000 queries/day free tier, resets **midnight Pacific**. **This limit is NOT raisable — confirmed 2026-08-08**: Rebekah checked the Cloud Console quota page for the project and there is no path to request more than 1K. Treat 1,000/day as a permanent hard ceiling when sizing any Google-dependent lane; do not plan work that assumes a quota increase. The fixed allowance is split **500 to `upcoming-releases`** (its `GOOGLE_MAX`, halved from 800) and the remainder to the description tier + cover cascade, per Rebekah's 500/500 call. Use `skipGoogleBooks` in `enrichBook()` to opt a lane out. **Budget-guarded since 2026-08-08:** `searchGoogleBooksDetailed()` — the single choke point — calls `consumeApiQuotaShared("google_books", GOOGLE_BOOKS_DAILY_MAX, "pacific")` and returns `quotaExhausted` instead of firing the request. Before this, GB was completely ungoverned while THREE consumers spent it (`upcoming-releases` at 800/day, the enrichment cover cascade, and the new description tier), and the error log showed daily 429s on 7 of the last 8 days — every lane read the resulting 503 as an upstream outage rather than "someone else spent your budget". The counter keys on the **Pacific** day, not UTC like Brave's, because that is when Google actually resets; a UTC key would run 7-8h out of phase with the real quota. `GOOGLE_BOOKS_DAILY_MAX` (default 950, leaving ~50 for ad-hoc runs) is the one knob — raise it when the Cloud Console Queries-per-day quota is raised.
  - **Descriptions come from Google Books too, as of 2026-08-08** (`fetchGoogleBooksDescription`, enrich-book Phase 1.6, between ISBNdb and the `skipContentSearch` hatch). CLAUDE.md and the description-refresh SKILL had both claimed a "OL → ISBNdb → Google Books" description cascade for months, but no GB description fetch existed anywhere — GB was wired for covers only. Measured 25% hit rate at ~0.5s/book against the 40 oldest stale books (the hardest cases, all of which had already failed OL and ISBNdb repeatedly). Sitting above the hatch means the Brave-free lanes (discovery, breadth-import, upcoming-releases) get descriptions too instead of landing books blank. ISBN-13 → ISBN-10 → title+author, first hit wins; the title+author fallback requires BOTH title and author to corroborate, since `q=` free text will cheerfully return a different book with the same name.
...
### Related
- **Edition-variant merges (`<Title> Paperback Deluxe Limited Edition` → `<Title>`) — added 2026-08-14.** Publishers ship the same work under decorated titles and each lands as its own `books` row; Rebekah's standing rule is that these are editions of the canon book, not separate entries. `process-reports.ts` strips edition decoration (`stripEditionSuffix`) before sibling matching, and the candidate-prefix query uses the STRIPPED title — slicing the raw title produced `'Unravel Me Paperbac%'`, which cannot match the canonical "Unravel Me", so the canon was excluded from the candidate set and every such report died as "no sibling found" rather than even being flagged for a human. Seven of these (the Tahereh Mafi Shatter Me set) sat open a week and were merged by hand. Decoration words only — `Collection`, `Box Set`, `Omnibus` and volume numbers are deliberately NOT stripped, because those are genuinely different products. **An undecorated title outranks everything in canonical scoring** (a deluxe row with more shelf activity must never win and cause the real book to be deleted). Auto-merge is still gated on the dupe having ZERO user rows, because `mergeDupeIntoCanonical()` does not move user rows — it deletes on the strength of that contract. Do NOT swap that gate for `findUserOverlap()` without also adding `UPDATE book_id` row-moving first; that combination is exactly the 2026-07-30 data-loss incident. Dupes carrying user rows are meant to fall through to `merge-ambiguous` and be handled by the supervised `replay-dedup-both.ts`.
- **The triage queue must be WORKED, not just reported (rule added 2026-08-14).** `process-reports.ts` correctly declines judgment-heavy reports, but for months nothing picked up the residue and "filed in /admin/issues" was treated as done. The queue reached 36 open with the oldest at **105 days**. The script now stamps every open report with its age and leads the nightly summary with a `## ⚠️ Stale — open 14+ days` section; the `nightly-report-triage` SKILL.md requires the agent to fix those or state why not. When you touch this script, keep the aging output — it is the only thing that distinguishes a queue being managed from one rotting.
...
## Watch Out For
- **iOS releases: read `docs/ios-release-checklist.md` BEFORE archiving, and run `./native-ios/preflight-archive.sh <archive> <build#>` before every upload.** `native-ios/project.pbxproj` is gitignored AND regenerated by `xcodegen generate`, so any build setting kept there silently reverts on the next generate — this has shipped a build with no app icon, a Release plist with `NSAllowsArbitraryLoads`, a build-number bump that did nothing, and an upload rejected for missing orientations. Settings that must ship belong in `native-ios/project.yml`. Note also that `INFOPLIST_KEY_*` build settings are ignored while `GENERATE_INFOPLIST_FILE=NO` — those keys must be real `info.properties` entries. A build in `PROCESSING` is invisible to the ASC API and shows no icon in the web UI; that is not evidence of failure (Apple allows up to 24h). Apple's emails go to the Account Holder, `rebekah@clara.agency`.
