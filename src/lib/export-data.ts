// Shared export generators — used by BOTH the cookie-auth /api/export
// route (web) and the bearer-auth /api/v1/export route (native app).
...
export async function generateJSON(userId: string) {
  const [
    bookStates,
    sessions,
    ratings,
