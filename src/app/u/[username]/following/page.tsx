...
import { getFollowing, isFollowing } from "@/lib/queries/follows";
...
export async function generateMetadata({
  ...
  return {
    title: `${displayName} is Following | tbr*a`,
    robots: { index: false },
  };
}
  ...
  if (session) {
    await Promise.all(
      following.map(async (f) => {
        const isFollowingUser = await isFollowing(session.userId, f.userId);
        followingMap.set(f.userId, isFollowingUser);
      })
    );
  }
  ...
  return (
    <div className="min-h-screen">
      <div className="px-4 py-4 border-b border-border/50">
        ...
        <p className="text-xs text-muted">{following.length} following</p>
      </div>

      <div className="px-4 py-4 space-y-2">
        {following.length === 0 && (
          <p className="text-sm text-muted text-center py-8">Not following anyone yet</p>
        )}
        {following.map((f) => {
