...
export function ReviewSummary({
  ...
  return (
    <section className="mt-8 space-y-4">
      <h2 className="text-xl font-bold uppercase tracking-wide text-neon-blue">
        What Readers Think
      </h2>
      ...
      {data.topReviews.length > 0 && (
        <div className="space-y-2.5">
          {data.topReviews.map((review) => (
            <Link
              key={review.id}
              href={`/book/${bookId}/reviews#review-${review.id}`}
      ...
      <Link
        ...
        className="flex items-center justify-center gap-1 text-sm font-medium text-purple-400 hover:text-purple-300 transition-colors py-1"
      >
        View all {data.totalReviewCount}{" "}
        {data.totalReviewCount === 1 ? "review" : "reviews"}
        <svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
