...
export async function saveReview(payload: ReviewPayload) {
  ...
  const { bookId, overallRating, didNotFinish, dnfPercentComplete, reviewText, mood, dimensionRatings, dimensionTags } = payload;
  ...
  const cwTags = dimensionTags["content_warnings"] ?? [];
