...
- **Token lifetime: "never log out" via refresh-token rotation — IMPLEMENTED 2026-06-30.** 1-hour access JWT + 60-day rotating refresh token (`auth_refresh_tokens` table, `src/lib/auth-refresh.ts`, `/api/v1/auth/{login,refresh,logout}`). Only the SHA-256 hash of each refresh token is stored; rotation-on-use limits a stolen token to one use; logout / `{all:true}` revoke server-side. On-device tokens live in the iOS Keychain; the password is never in the token (stays bcrypt-hashed).
  - **⚠️ Deploy prerequisite:** the `auth_refresh_tokens` table must be created on BOTH local (`npm run db:push`) and **Turso before deploying** this code (per the CLAUDE.md rule — new tables land on Turso first). Migration: `drizzle/0003_auth_refresh_tokens.sql`.
